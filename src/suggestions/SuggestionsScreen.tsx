...
export default function SuggestionsScreen() {
  ...
  async function getAISuggestions(intent: SuggestionIntent = 'general') {
    ...
    const req: SuggestionRequest = { 
      ...
      style, 
      intent,
      onStreamChunk: (chunk) => setStreamText(prev => prev + chunk)
    };
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {loading && streamText && (
        <div className="bg-white/5 border border-indigo-500/30 rounded-2xl p-6 italic text-slate-400 text-sm animate-pulse">
          <div className="flex items-center gap-2 mb-2 text-indigo-400 non-italic font-bold">
            <BrainCircuit className="w-4 h-4" />
            教練正在分析中...
          </div>
          {streamText}
